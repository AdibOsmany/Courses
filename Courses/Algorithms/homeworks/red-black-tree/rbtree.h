...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        if(find(key)!=end()){
            ...
            x=new Node(key,key_value.second);
            x->parent=y;
            if(y==nullptr){
                this->root_=x;
            }
...
private:
        ...
        while((z->parent!=nullptr)&&(z->parent->color==RED)){
            ...
            if (z->parent==z->parent->parent->left){
                if((z->parent->parent->right != nullptr) &&(z->parent->parent->right->color==RED)){
                    std::cout<<"Case 1a applied to "<<z->key()<<std::endl;
                    ...
                    if(z==z->parent->right){
                        std::cout<<"Case 2a applied to "<<z->key()<<std::endl;
                        z=z->parent;
                        left_rotate(z);
                    }
                    std::cout<<"Case 3a applied to "<<z->key()<<std::endl;
                    z->parent->color=BLACK;
                    z->parent->parent->color=RED;
                ...
                if((z->parent->parent->left != nullptr) &&(z->parent->parent->left->color==RED)){
                    std::cout<<"Case 1b applied to "<<z->key()<<std::endl;
                    ...
                    if(z==z->parent->left){
                        std::cout<<"Case 2b applied to "<<z->key()<<std::endl;
                        z=z->parent;
                        right_rotate(z);
                    }
                    std::cout<<"Case 3b applied to "<<z->key()<<std::endl;
                    z->parent->color=BLACK;
                    z->parent->parent->color=RED;
    ...
    size_t leaf_count(Node<K, V> *node) const {
        ...
        if(node==nullptr){
            ...
            int h1=leaf_count(node->left);
            int h2=leaf_count(node->right);
            return(h1+h2);
        }
    }
    ...
    size_t internal_node_count(Node<K, V> *node) const {
        // TODO
        return this->size_-leaf_count(node);
    }
    ...
    size_t width(Node<K, V> *node, size_t level) const {
        ...
        if(node==nullptr){
            ...
            size_t h1=width(node->left,level-1);
            size_t h2=width(node->right,level-1);
            return h1+h2;
        }
    }
    ...
    size_t null_count(Node<K, V> *node) const {
        ...
        if(node==nullptr){
            ...
            size_t h1=null_count(node->left);
            size_t h2=null_count(node->right);
            return(h1+h2);
        }
    }
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
        ...
        if(node==nullptr){
            ...
            size_t h1=sum_levels(node->left,level+1);
            size_t h2=sum_levels(node->right,level+1);
            return (h1+h2+level);
        }
    }
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const {
        ...
        if(node==nullptr){
            ...
            size_t h1=sum_null_levels(node->left,level+1);
            size_t h2=sum_null_levels(node->right,level+1);
            return (h1+h2);
        }
    }
};
