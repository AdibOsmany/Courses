...
;; 2 PTS
(define (rotate c)
  (list c (list (cadr c) (* -1 (car c)) )  (list (* -1 (car c)) (* -1 (cadr c)) ) (list (* -1 (cadr c)) (car c) ))
)
...
;; 1 PTS
(define (piecewise x)
  (cond ((< x 0) (+ x 1)) ((= x 0) (+ x 1)) (else (+ 1 (* 2 (* x x)))))
)
...
;; 1 PTS
(define (sign x)
  (cond ((> x 0) (car '("positive"))) ((< x 0) (car '("negative"))) (else (car '("zero"))))
)
...
(define (gen-fib a b n)
  (if (= n 0)
      ...
      (if (= n 1)
          b
          (if (> n 1)
              (+ (gen-fib a b (- n 1)) (gen-fib a b (- n 2)))
              0
              )
          )
      )
