...
(define (p-help pea [recurs (succ (succ z))])
 (cond
  [ (or (p= z pea) (p= z (pred pea))) #f]
  [(p= recurs pea) #t]
  [(p-divides? recurs pea) #f]
  [else (p-help pea (succ recurs))])
)
