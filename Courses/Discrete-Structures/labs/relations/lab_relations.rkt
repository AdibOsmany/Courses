...
#|-------------------------------------------------------------------------------|
 |                  Lab 6: Relations Part II (20 PTS +6 EC)                      |
 |-------------------------------------------------------------------------------|#
...
(define (comp S T)
  (if (null? S)
      '()
      (if (equal? (cadr T) (car (car S)))
          (append (list (cons (car T) (cdr (car S)))) (comp (cdr S) T))
          (comp (cdr S) T)
          )))
...
(define (compose S R)
  (if (or (null? R) (null? S))
          '()
      (make-set (append (comp S (car R)) (compose S (cdr R))))
))
...
(define (equivalence? n R)
  (if (and (reflexive? n R) (symmetric? R)  (transitive? R))
      #t
      #f
 ))
...
#| Even more fun!
 | Implement "function?" which takes a relation R which returns true if the relation is also a function. That is one
 | input maps to at most one output. 
 |
 | Examples:
 |   (function? '((1 2) (2 1)) )
 |     -> #t    [This applies as a function.]
 |   (function? '((3 3) (1 1) (2 2)) )
 |     -> #t    [y = x is also a function]
 |   (function? '((3 3) (2 1) (2 2) (5 5)) )
 |     -> #f    [... but this relation isn't. 2 maps to both 1 and 2.]
 |   (function? '((1 2) (2 2) (3 2) (4 2)) )
 |     -> #t    [We can have all values map to the same thing all we like!]
 |
 |#
