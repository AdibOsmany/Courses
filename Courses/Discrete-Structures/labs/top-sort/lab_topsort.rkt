...
#| In this lab, we'll implement topological sort,
 ...
 |
 |
 | Topological sort is an algorithm which lists the vertices of a digraph
...
(define (ts G [lis '()])
  (if (equal? empty G)
      (reverse lis)
      (ts (pop-vertex G (find-top G)) (cons (find-top G) lis) ))
