...
(define (CRT-helper L m)
  ...
  (if (= 0 (length L))
      0
  (if (= 1 (length L))
      (* (car (car L)) (quotient m (cadr (car L))) (mod-inverse (quotient m (cadr (car L))) (cadr (car L))))
      (+ (* (car (car L)) (quotient m (cadr (car L))) (mod-inverse (quotient m (cadr (car L))) (cadr (car L)))) (CRT-helper (cdr L) m))
      ))
...
(define (CRT L)
  ...
  (if (CRT-exists? L)
      (modulo (CRT-helper L (find-m L)) (find-m L))
      -1)
)
