...
;; 5 PTS
(define (wj a b g [fa 0] [fb 0] [c '((0 0))])
  (if (= fb g)
      (reverse c)
      (if (= fb b)
          (wj a b g fa 0 [cons (list fa 0) c])
          (if (= 0 fa)
              (wj a b g a fb [cons (list a fb) c])
              (if (> (+ fa fb) b)
                  (wj a b g [- fa (- b fb)] b [cons (list (- fa (- b fb)) b) c])

                      (wj a b g 0 [+ fb fa] [cons (list 0 (+ fb fa)) c]))))))
