...
(define (mod-exp b e m)
  ...
  (if (= e 0)
      1
      (if (and (> e 0) (= 0 (modulo e 2)))
          (modulo (expt (modulo (expt b (quotient e 2))  m) 2) m)
       
             (modulo (* (modulo (expt b (- e 1))  m) b) m) 
         )
    )
)
...
(define (eh lis m e)
  (define c (char->integer (car lis)))
  (if (= 1 (length lis))
      (list (mod-exp c e m))
      (append (list (mod-exp c e m))  (eh (cdr lis) m e))
      )
  )
...
(define (encrypt plaintext key)
  ...
  (eh (string->list plaintext) (car key) (cadr key))
...
(define (he n m e)
  (define p (car n))
  (if (= 1 (length n))
      (list (integer->char (mod-exp p e m)))
      (append (list (integer->char (mod-exp p e m)))  (he (cdr n) m e))
      )
  )
...
(define (decrypt ciphertext key)
  ...
  (list->string (he ciphertext (car key) (cadr key)))
)
