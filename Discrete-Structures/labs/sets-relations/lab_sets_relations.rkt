...
;; 3 PTS
(define (intersect S1 S2 [t (length S1)] [p (length S2)])
  (if (null? S1)
      S1
      (if (null? S2)
          S2
          (if (= t p)
              (if (element? S1 (car S2))
                  (cons (car S2) (intersect S1 (cdr S2)))
                  (intersect S1 (cdr S2)))
              (if (> t p)
                  (if (element? S1 (car S2))
                      (cons (car S2) (intersect S1 (cdr S2)))
...
;; 1 PTS
(define (dis? S1 S2 [t (intersection S1 S2)])
  (if (null? t)
      #t
      #f)
)
...
;; 3 PTS
(define (subs? S1 S2 [ t (intersection S1 S2)])
      (if (= (length S1) (length t))
          #t
          #f))
...
;; 1 PTS
(define (settt S1 S2 [ t (intersection S1 S2)])
  (if (= (length S1) (length S2) (length t) )
          #t
          #f))
...
(define (sym S1 S2 [t (difference S1 S2)] [p (difference S2 S1)])
    (if (null? S1)
      S2
      (if (null? S2)
          S1
...
;; 3 PTS
(define (verse R)
  (cons (cadr R) (list (car R))))

(define (converse R)
  (if (null? R)
      R
     (cons (verse (car R)) (converse (cdr R)))
))
